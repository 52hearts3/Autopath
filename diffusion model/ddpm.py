...
# 假设 model, gaussian_diffusion, timesteps 已定义
optimizer = torch.optim.Adam(model.parameters(), lr=1e-4, weight_decay=1e-5)  # 初始学习率适应大 batch_size
scaler = torch.cuda.amp.GradScaler()
model.load_state_dict(torch.load(r'he_batch345-3.pth'))
model.to(device)

# 梯度累积参数
accumulation_steps = 512 # 累积8次 batch_size=2，模拟 batch_size=256
timesteps = 1000
epochs = 500

# 动态学习率调度
from torch.optim.lr_scheduler import CosineAnnealingLR
# scheduler = CosineAnnealingLR(optimizer, T_max=len(dataloader) // accumulation_steps, eta_min=1e-6)  # 每个epoch内学习率从4e-4降到1e-6
scheduler = CosineAnnealingLR(optimizer, T_max=len(dataloader), eta_min=1e-6)

# 训练循环
MASK_DROP_PROB = 0.35          # mask被置为全零的概率，常见范围 0.15~0.30
# MASK_DROP_WARMUP_EPOCHS = 10  # 如果想做warmup可打开下面代码

for epoch in range(epochs):
    print(f"Epoch {epoch + 1}/{epochs}")

    # 如果想做概率warmup（早期少drop，后期接近目标概率），可取消注释
    # if epoch < MASK_DROP_WARMUP_EPOCHS:
    #     current_drop_prob = MASK_DROP_PROB * (epoch + 1) / MASK_DROP_WARMUP_EPOCHS
    # else:
    #     current_drop_prob = MASK_DROP_PROB
    # print(f"  current mask drop prob = {current_drop_prob:.3f}")
    current_drop_prob = MASK_DROP_PROB   # 直接使用固定概率（最常用）

    dataset.shuffle_indices()   # 假设你的dataset有这个方法打乱顺序
    ...
    optimizer.zero_grad()  # 每个epoch开始清零（保险起见）

    for step, batch in enumerate(pbar):
        mask_image = batch["mask"].to(device)
        he_image   = batch["he"].to(device)
        current_batch_size = he_image.shape[0]

        # ───────────── 关键：随机丢弃 control (mask置零) ─────────────
        if torch.rand(1, device=device).item() < current_drop_prob:
            mask_image.zero_()               # 原地清零，更高效
            # 或者： mask_image = torch.zeros_like(mask_image)

        # 也可以每个样本独立drop（更随机，但batch内风格可能不一致）
        # drop = torch.rand(current_batch_size, device=device) < current_drop_prob
        # mask_image = mask_image * (~drop.view(-1, 1, 1, 1))

        t = torch.randint(0, timesteps, (current_batch_size,), device=device).long()

        with torch.cuda.amp.autocast():
            cond = torch.zeros(current_batch_size, 512, device=device)

            losses = gaussian_diffusion.train_losses(
                model=model,
                x_start=he_image,
                cond=cond,
                t=t,
                control_img=mask_image,    # ← 这里mask有概率已经是全零
                ssim_weight=0
            )
        ...
        # 累积步数达到accumulation_steps 或 最后一个batch → 更新
        if (step + 1) % accumulation_steps == 0 or (step + 1) == len(dataloader):
            ...
            # 学习率调度（大多数情况下放在这里比较合理）
            scheduler.step()

        # 记录损失（使用 .item() 转为python标量）
        total_loss      += losses["total_loss"].item()
        total_mse_loss  += losses["mse_loss"].item()
        total_ssim_loss += losses["ssim_loss"].item()
        steps += 1

        # 更新进度条显示
        avg_total = total_loss / steps
        avg_mse   = total_mse_loss / steps
        avg_ssim  = total_ssim_loss / steps
    ...
    # epoch 结束，保存模型（可按需改成每N个epoch保存，或保存最好模型）
    save_path = f"he_batch345-3-2.pth"
